...
class Reporter:
    ...
    def save_reports(
        ...
        Args:
            success: Whether run was successful (ignored — derived from summary counts)
            summary: Processing summary with processed/failed counts
            error_message: Error message if fatal failure
        ...
        if failed == 0 and processed > 0:
            ...
            self._save_partial_success_report(summary, profiles_with_fixes)
        else:
    ...
    def _save_success_reports(
        ...
        if profiles_with_fixes:
            ...
            self.output_writer.write_report(
                profiles_report, "PROFILES-WITH-FIXES.md"
            )

        logger.info("Reports saved successfully")

    def _save_partial_success_report(
        self,
        summary: Dict[str, Any],
        profiles_with_fixes: Optional[List[Dict[str, str]]] = None,
    ) -> None:
        """Save partial success report when some images failed."""
        report = self._generate_partial_success_report(summary)
        self.output_writer.write_report(report, "PARTIAL_SUCCESS.md")
        ...
        logger.info("Partial success report saved")
    ...
    def _generate_partial_success_report(self, summary: Dict[str, Any]) -> str:
        """Generate partial success report content."""
        ...
        if processed > 0:
            avg_time = processing_time / processed
            report_lines.extend(
                [
                    "## Performance Metrics",
